...
async fn run_backtest_mode(args: &Args) -> Result<()> {
    use backtest::{
        fetch_binance_klines, print_results_table, print_single_result, run_backtest,
        run_param_sweep, StrategyParams,
    };
    use chrono::{NaiveDate, Utc};

    info!("🧪 Starting backtest mode");

    // === Парсинг дат с фиксом ===
    let parse_date = |date_str: &str| -> Result<i64> {
        // Если это уже таймстамп (число)
        if let Ok(ts) = date_str.parse::<i64>() {
            return Ok(ts * 1000); // секунды → миллисекунды
        }

        // Парсим как дату "YYYY-MM-DD"
        let date = NaiveDate::parse_from_str(date_str, "%Y-%m-%d")
            .map_err(|e| anyhow::anyhow!("Invalid date '{}': {}", date_str, e))?;

        // Конвертируем в UTC timestamp (начало дня)
        let dt = date
            .and_hms_opt(0, 0, 0)
...
async fn run_live_mode(args: &Args) -> Result<()> {
    ...
    let mut trailing_stop = TrailingStop::new(1.5);
                ...
                if position == Position::None {
                    ...
                    if bullish_cross && oversold {
                        ...
                        if !bullish_cross && !bearish_cross {
                            info!(
                                "⏸️ No entry: MACD no crossover (hist: {:.4}, prev: {:.4})",
                                histogram, prev
                            );
